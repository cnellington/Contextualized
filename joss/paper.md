...
For example, if we wished to model human development and disease in the context of human genetics, there are 11,500,000 known single-nucleotide polymorphisms in humans, implying $2^{11,500,000}$ genetic contexts, but only about $2^{37}$ people have ever existed.
Observing every possible context, much less drawing sufficient samples from each context, is impossible.

`Contextualized ML` presents a solution to context-specific modeling even when the number of contexts vastly exceeds the number of observed samples by learning to interpolate and extrapolate between contexts.
A recent study applies this property to contextualize models of gene regulation in cancer with 1600 patient-specific genetic variants, learning to interpolate between $2^{1600}$ possible contexts with only $2^13$ samples [@ellington_contextualized_2023].
...
Sample-specific statistical models quantify the randomness and the structure of the systems underlying each data point in terms of a probability distribution.
By comparing the sample-specific models, we can quantify how these systems vary to produce heterogeneity in data, identify the most important sources of heterogeneity, and determine what heterogeneity is explained by contextual factors and what is caused by random noise.
