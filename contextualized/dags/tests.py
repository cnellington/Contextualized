...
class TestDAGs(unittest.TestCase):
    ...
    def test_notmad(self):
        ...
        k = 5
        sample_specific_loss_params = {'l1': 0., 'init_alpha': 1e-1, 'init_rho': 1e-2}
        # loss_params = {'l1': 1e-2, 'alpha': 1e-1, 'rho': 1e-2}
        archetype_loss_params = {'l1': 0., 'alpha': 1e-1, 'rho': 1e-2}
        init_mat = np.random.uniform(-0.01, 0.01, size=(k, self.X_train.shape[-1], self.X_train.shape[-1])) #np.zeros((k, X_train.shape[-1], X_train.shape[-1])) #
        notmad = NOTMAD(
            self.C_train.shape, self.X_train.shape, k,
            sample_specific_loss_params, archetype_loss_params,
            n_encoder_layers=2, encoder_width=32,
            activation='linear', init_mat=init_mat,
            learning_rate=1e-3, project_archs_to_dag=True, # TODO: should this be variable?
        ...
        notmad_preds_train = notmad.predict_w(self.C_train, project_to_dag=True).squeeze()
        notmad_preds = notmad.predict_w(self.C_test, project_to_dag=True).squeeze()
        param_mse_init = mse(notmad_preds_train, self.W_train)
        dag_mse_init = mse(self._dag_pred(self.X_train, notmad_preds_train), self.X_train)
        
        notmad.fit(
            self.C_train, self.X_train, batch_size=self.batch_size, epochs=self.epochs, 
            es_patience=2, verbose=1
        )
        notmad_preds_train = notmad.predict_w(self.C_train, project_to_dag=True).squeeze()
        notmad_preds = notmad.predict_w(self.C_test, project_to_dag=True).squeeze()
        param_mse_trained = mse(notmad_preds_train, self.W_train)
        dag_mse_trained = mse(self._dag_pred(self.X_train, notmad_preds_train), self.X_train)
