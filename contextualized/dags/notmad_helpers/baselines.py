...
class DynamicAlphaRho(Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        pred = np.squeeze(
            self.model.predict(
                np.expand_dims(self.C_train[np.random.choice(self.C_train.shape[0])], 0)
            )
        )
        # pred = trim_params(pred, thresh=0.1)
        my_dag_loss = DAG_loss(
            pred, self.model.alpha.numpy(), self.model.rho.numpy()
        )  # TODO: should be measured over batch
        self.model.W.W.assign(
            self.model.W.W * (1 - np.eye(self.model.W.W.shape[0]))
...
class NOTEARS:
    ...
    def __init__(
        ...
        self.model.W = self.W
        my_loss = lambda x, y: NOTEARS_loss(
            x, y, loss_params["l1"], self.model.alpha, self.model.rho
        )
        self.model.compile(loss=my_loss, optimizer=self.opt, metrics=self.metrics)
...
class ClusteredNOTEARS:
    ...
    def __init__(
        ...
        self.notears_models = [
            NOTEARS(loss_params, context_shape, W_shape, learning_rate)
    ...
    def fit(
        ...
        for clust in loop:
            ...
            if np.sum(ct_idxs) < 2:
                self.notears_models[clust].set_w(
                    np.zeros_like(self.notears_models[clust].get_w())
                )
            else:
                C_ct = C[ct_idxs]
                X_ct = X[ct_idxs]
                self.notears_models[clust].fit(
                    C_ct,
                    X_ct,
                    epochs,
                    batch_size,
                ...
                self.notears_models[clust].set_w(
                    project_to_dag(self.notears_models[clust].get_w())[0]
                )
