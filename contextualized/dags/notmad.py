...
from contextualized.dags.notmad_helpers.baselines import (
    save_clusterednotears,
    load_clusterednotears,
)
...
class ProjectToDAG(Callback):
    ...
    def project_low_rank(self, A, B, mask):
        ...
        arches = np.vstack(
            ([graph_utils.project_to_dag(a.dot(np.squeeze(B)))[0] for a in A])
        )
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for layer in self.model.layers:
            if hasattr(layer, "archetypes"):
                explainer = layer
        try:
            A_new, B_new = self.project_low_rank(
                explainer.A.numpy(), explainer.B.numpy(), explainer.mask.numpy()
            )
...
class UpdateCompatMat(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i in range(explainer.k):
            for j in range(i):
                # for k in range(j):
                if graph_utils.is_dag(
                    archs[i] + archs[j]
                ):  # graph_utils.is_dag(graph_utils.trim_params(archs[i] + archs[j], thresh=0.01)):
                    compat_mat[i, j] = 1
                    compat_mat[j, i] = 1
...
class Explainer(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        if self.use_compat:
            if init_compat is None:
                init_compat = (
                    np.eye(self.k),
                )  # initialize to only picking 1 archetype
            self.compatibility_mat = self.add_weight(
                "compat",
                shape=(self.k, self.k),
        ...
        if self.d2 == self.d1:  # Full-Rank
            ...
            if init_archs is not None:
                ...
                init_A = np.array(
                    [
                        init_A
                        + np.random.binomial(0, 0.1, size=init_A.shape)
                        * np.random.normal(0, 0.1, size=init_A.shape)
    ...
    def archetypes(self):
        ...
        try:
            ...
            )  # tf.keras.backend.batch_dot(self.A, self.B)
        except:
            archs = self.archs
    ...
    def call(self, subtype):  # Subtype is of shape (None x k)
        ...
        if self.use_compat:
            ...
            subtype = tf.multiply(
                tf.tensordot(max_subtype, self.compatibility_mat, axes=1), subtype
            )
...
class NOTMAD:  # TODO: Only difference between low-rank and full-rank version should be the Explainer.
    ...
    def __init__(
        ...
        self.model = tf.keras.models.Model(
            inputs=(self.context, self.base_W), outputs=self.outputs
        )
        ...
        self.my_loss = lambda x, y: NOTEARS_loss(
    ...
    def fit(
        ...
        if (
            ...
            and not self.update_compat_by_grad
            and not self.freeze_compat
        ):
    ...
    def progressive_fit(
        ...
        for i, grouping in enumerate(groupings):
            if verbose:
                print(f"Grouping {i+1}/{len(groupings)}")
            labels = np.unique(grouping)

            if len(labels) == len(grouping):  # Normal fit for individual grouping
                self.model.fit(
                    C, X, batch_size=1, epochs=epochs_per_grouping, verbose=verbose
                )
                continue
            ...
            for epoch in range(epochs_per_grouping):  # Group fit otherwise
                for j, label in enumerate(labels):
                    ...
                    sample_idx = np.random.choice(
                        np.arange(len(C_group)), size=max_samples
                    )
