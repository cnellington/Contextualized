...
class TestContextualizedRegressorNormalization(unittest.TestCase):
    """
    Test Normalization for Regression models.
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def test_normalization_and_prediction_pipeline(self):
        ...
        np.testing.assert_allclose(model.scaler_C.mean_, np.mean(C_train, axis=0), atol=1e-1)
        np.testing.assert_allclose(model.scaler_C.scale_, np.std(C_train, axis=0), atol=1e-1)
        np.testing.assert_allclose(model.scaler_X.mean_, np.mean(X_train, axis=0), atol=1e-1)
        np.testing.assert_allclose(model.scaler_X.scale_, np.std(X_train, axis=0), atol=1e-1)
